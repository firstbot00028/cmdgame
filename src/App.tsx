...
import { ShoppingCart, Car, Image as ImageIcon, Settings, CheckCircle2, X, Plus, LogOut, MessageCircle, ArrowRight, ShieldCheck, Trash2, Tag } from 'lucide-react';
...
const Admin = () => {
  ...
  const toggleSoldOut = async (product: Product) => {
      const newStatus = product.status === 'Sold Out' ? 'Available' : 'Sold Out';
      try {
            ...
            <div className="bg-luxury-dark border border-gray-800 p-8 rounded-3xl">
              ...
              {waStatus.status === 'connected' ? (
                <div className="text-center py-10">
                  ...
                  {waStatus.user && (
                    <p className="text-luxury-gold font-mono text-sm mb-4">
                      Account: {waStatus.user.id.split(':')[0]}
                    </p>
                  )}
                  <p className="text-gray-400 mb-8">Your WhatsApp is active and ready to receive orders to your chosen admin number.</p>
                  <button onClick={handleLogoutWA} className="text-red-500 border border-red-500/30 px-6 py-2 rounded-lg hover:bg-red-500 hover:text-white transition-all font-bold text-sm">
                    Disconnect Account
                  </button>
                </div>
              ) : (
                <div className="text-center py-6">
                  {waStatus.qr ? (
                    <div className="space-y-6">
                      ...
                      <div className="flex items-center justify-center gap-2 text-luxury-gold mt-4">
            ...
            <div className="bg-luxury-dark border border-gray-800 p-8 rounded-3xl">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-[10px] text-gray-600 mt-2 font-medium">Include country code without + (e.g. 91 for India)</p>
                </div>
              </div>
            </div>
          </motion.div>
        ) : (
          <motion.div key="products" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-10">
            <div className="bg-luxury-dark border border-gray-800 p-8 rounded-3xl">
               ...
               <form onSubmit={handleAddProduct} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    {newProduct.imageUrl && (
                      <div className="mt-4 p-2 bg-luxury-black rounded-lg border border-gray-800 flex items-center gap-4">
                        ...
                        <div className="flex-1 overflow-hidden">
                           <span className="text-[10px] text-gray-500 truncate block">{newProduct.imageUrl}</span>
                        </div>
                  ...
                  <div className="md:col-span-2">
                    ...
                    <textarea 
                      required
                      className="w-full bg-luxury-black border border-gray-800 rounded-xl px-4 py-3"
                      placeholder="Details about the logo or car..."
                      rows={3}
                      value={newProduct.description}
                      onChange={e => setNewProduct({...newProduct, description: e.target.value})}
                    />
                  </div>
                  <button className="md:col-span-2 bg-luxury-gold text-luxury-black py-4 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-gold-light transition-all">
            ...
            <div className="space-y-4">
               <h3 className="font-serif text-2xl mb-6">Existing Collection</h3>
               <div className="grid grid-cols-1 gap-4">
                 {products.map(p => (
                   <div key={p.id} className="bg-luxury-dark border border-gray-800 p-4 rounded-2xl flex items-center gap-4">
                      <img src={p.imageUrl} className="w-16 h-16 object-cover rounded-lg" />
                      <div className="flex-1">
                        <h4 className="font-bold">{p.name}</h4>
                        <div className="flex items-center gap-3">
                          <span className="text-luxury-gold font-mono">₹{p.price}</span>
                          <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded ${p.status === 'Sold Out' ? 'bg-red-500/20 text-red-500' : 'bg-green-500/20 text-green-500'}`}>
                            {p.status || 'Available'}
                          </span>
                        </div>
                      </div>
                      <div className="flex gap-2">
                        <button 
                          ...
                          title={p.status === 'Sold Out' ? 'Mark Available' : 'Mark Sold Out'}
                          className={`p-3 rounded-xl transition-all ${p.status === 'Sold Out' ? 'bg-green-500/10 text-green-500 hover:bg-green-500 hover:text-white' : 'bg-orange-500/10 text-orange-500 hover:bg-orange-500 hover:text-white'}`}
                        >
                          <Tag size={18} />
                        </button>
                        <button 
